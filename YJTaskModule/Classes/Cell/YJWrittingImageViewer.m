...
@interface YJWrittingImageViewer ()<UIScrollViewDelegate>
...
@end
@implementation YJWrittingImageViewer
- (instancetype)initWithFrame:(CGRect)frame images:(NSArray *)images atIndex:(NSInteger)index{
    if (self = [super initWithFrame:frame]) {
        self.imageCount = images.count;
        [self addSubview:self.scrollView];
        for (int i = 0; i < self.imageCount; i++) {
            YJWrittingImageBrowserScrollView *photoView = [[YJWrittingImageBrowserScrollView alloc] initWithFrame:CGRectMake(LG_ScreenWidth*i, 0, LG_ScreenWidth, LG_ScreenHeight)];
...
+ (void)showWithImageUrls:(NSArray *)imageUrls atIndex:(NSInteger)index{
    YJWrittingImageViewer *imgViewerView = [[YJWrittingImageViewer alloc] initWithFrame:[UIScreen mainScreen].bounds images:imageUrls atIndex:index];
    [imgViewerView show];
}
+ (void)showWithImages:(NSArray *)images atIndex:(NSInteger)index{
    YJWrittingImageViewer *imgViewerView = [[YJWrittingImageViewer alloc] initWithFrame:[UIScreen mainScreen].bounds images:images atIndex:index];
    [imgViewerView show];
}
- (void)show{
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    if (fabs(_dragOffsetX - contenOffset) > LG_ScreenWidth / 2) {
        ...
        YJWrittingImageBrowserScrollView *photo = scrollView.subviews[self.currentIndex];
