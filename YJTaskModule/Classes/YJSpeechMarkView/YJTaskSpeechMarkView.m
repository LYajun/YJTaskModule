...
@interface YJTaskSpeechMarkView ()
@property(nonatomic,strong) UIView *maskView;
@end
@implementation YJTaskSpeechMarkView

+ (void)showSpeechMarkViewWithTitle:(NSString *)title{
    YJTaskSpeechMarkView *alertView = [[YJTaskSpeechMarkView alloc] initWithFrame:CGRectMake(0, 0, 200, 200)];
...
+ (void)showSpeechRecognizeView{
    ...
    gifImg.animationImages = [YJTaskSpeechMarkView yj_animationRecorderImages];
...
+ (void)dismiss{
   ...
   for (UIView *view in rootWindow.subviews) {
       if ([view isKindOfClass:[YJTaskSpeechMarkView class]]) {
           [(YJTaskSpeechMarkView *)view hide];
       }
   }
}

- (void)show{
    [YJTaskSpeechMarkView dismiss];
    UIWindow *rootWindow = [UIApplication sharedApplication].delegate.window;
